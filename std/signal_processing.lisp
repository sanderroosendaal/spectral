...
(defun lowpass-fn (params signal)
  ...
  (let* ((params-vec (ensure-vector params))
	 ...
	 (spec (fftw-ffi:fft-forward (map 'vector (lambda (x) (complex x 0.0d0)) data))))
    (unless (>= n 2)
      (error "lowpass: signal must have at least 2 samples, got ~A" n))
    (dotimes (k n)
      (let ((freq (if (<= k (floor n 2))
		      (* k fs (/ n))
		      (* (- n k) fs (/ n)))))
...
(defun highpass-fn (params signal)
  ...
  (let* ((params-vec (ensure-vector params))
	 ...
	 (spec (fftw-ffi:fft-forward (map 'vector (lambda (x) (complex x 0.0d0)) data))))
    (unless (>= n 2)
      (error "highpass: signal must have at least 2 samples, got ~A" n))
    (dotimes (k n)
      (let ((freq (if (<= k (floor n 2))
		      (* k fs (/ n))
		      (* (- n k) fs (/ n)))))
...
(defun bandstop-fn (params signal)
  ...
  (let* ((params-vec (ensure-vector params))
	 ...
	 (spec (fftw-ffi:fft-forward (map 'vector (lambda (x) (complex x 0.0d0)) data))))
    (unless (>= n 2)
      (error "bandstop: signal must have at least 2 samples, got ~A" n))
    (dotimes (k n)
      (let ((freq (if (<= k (floor n 2))
		      (* k fs (/ n))
		      (* (- n k) fs (/ n)))))
